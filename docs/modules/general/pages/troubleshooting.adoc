...
If the application is not showing up and errors like `t is not a function` are in the console, then most likely there is that the builder used in the project is not compatible with the Angular packages used in the project. To resolve the issue:
